...
class UpdateCommandValidator
{
    ...
    {
        if (
            $request->connector !== null && $request->connector !== null
            && ! $this->readConnectorRepository->exists($request->connector->getId())
        ) {
            throw CommandException::idDoesNotExist('connectorId', $request->connector->getId());
        }
    }
    ...
    {
        if (
            $request->graphTemplate !== null&& $request->graphTemplate->getId() !== null
            && ! $this->readGraphTemplateRepository->exists($request->graphTemplate->getId())
        ) {
            throw CommandException::idDoesNotExist('graphTemplateId', $request->graphTemplate->getId());
        }
    }
}
